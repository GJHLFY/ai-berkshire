...
Step 3 — 估值指标精确验算（PE/PB/ROE/ROIC/FCF Yield 等）：
...
> **核心原则**：报告不是大纲。每个分析模块必须有足够的**证据密度**——一段结论配一个表格或一组数据，而不是用总结性陈述替代分析过程。

### 各模块最低深度标准（输出前逐条自检）

**第二步（生意本质分析）**：

- [ ]  收入结构拆解表：至少列出 **4-6 项细分收入**，每项含金额、占比、同比变化
- [ ]  五年盈利能力趋势表：至少 **7 个指标 × 5 年**（营收、净利、增速、ROE、ROIC、毛利率/净息差、EPS）
- [ ]  商业模式画布：至少展开 **4 个维度**（如负债端、资产端、客户锁定、经营杠杆），每个维度用数据而非空话说明
- [ ]  好与坏的平衡：既要说"好在哪"，也要说"坏在哪"，用具体数据支撑两面

**第三步（护城河评估）**：

- [ ]  五类护城河逐一验证：每类配 **具体数据证据**（数字、百分比、排名），不能只写 ✅/❌ 符号
- [ ]  护城河趋势分析：**必须同时列出"变宽的证据"和"变窄的证据"**，各至少 2 条，附具体数据
- [ ]  10年预判必须具体：不能只说"大概率还在"，要说在什么条件下会怎样、什么能摧毁它

**第四步（风险管理）**：

- [ ]  失败路径表：至少列出 **5-7 种**具体失败路径，每项含概率、影响程度、详细说明
- [ ]  历史类比：至少找到 **2 个**历史上处于相似位置的公司，对比其结局，并从类比中提取具体教训
- [ ]  跨学科交叉验证：至少用 **2 个**非金融学科框架（如博弈论、均值回归、网络效应）分析该生意
- [ ]  空方论点：诚实呈现至少 **3 条**空方的核心论据，用数据和逻辑而非稻草人论证
- [ ]  偏误自查：至少列出 **2 项**自己在分析中可能犯的认知偏误

**第五步（管理层评估）**：

- [ ]  关键决策复盘表：至少 **3 个**时间节点/事件，每项含决策内容、结果、评分
- [ ]  资本配置分析：展开 **3 个以上方面**（分红纪律、回购时机、并购、研发回报率、融资历史）
- [ ]  诚信考察：正面和负面记录均须列出，不能只写优点

**第七步（估值）**：

- [ ]  反向DCF：**必须**用逆向推理——当前价格隐含了什么增长/ROE假设？用工具验算
- [ ]  三情景估值：**必须用 `financial_rigor.py three-scenario` 精确计算**，不能手算
- [ ]  与历史对比：给出 5 年或 10 年 PB/PE 分位，说明当前处于什么位置
- [ ]  与同行对比：与至少 **2 家**同行业可比公司的关键估值指标对比

**数据验证（第一步中执行）**：

- [ ]  关键数据交叉验证记录：**必须以附录形式**列出市值验算、PE/PB验算、营收/净利多源验证的具体结果
- [ ]  对于 A/B 级公司，**必须尝试纠正至少一处广为人知的错误数据**（如过时的零售AUM、错误的拨备覆盖率等），并在报告中说明

### 输出前自检清单（逐项打勾，全部通过才能写入文件）

在将报告写入文件前，逐项确认：

- [ ]  报告中每个分析模块都有**至少一个数据表格**（不是纯文字段落）
- [ ]  没有用"总结性陈述"替代分析过程（如只写"护城河强"而不列证据）
- [ ]  量化数据占比 ≥ 30%（粗略估计：报告中每 3 行文字至少有 1 行包含数字/表格/百分比）
- [ ]  估值部分所有计算均通过 `financial_rigor.py` 工具验算
- [ ]  附录包含数据交叉验证记录
- [ ]  报告中至少出现一次"但另一方面"（平衡论证的标志）
- [ ]  每个大师追问的引用块至少 3 句话，不是一句带过
...
10. **禁止浅层输出**：报告必须达到上方"分析深度强制要求"中的所有自检标准后才能写入文件。宁可分多次对话完成高质量报告，也不能为了节省篇幅而牺牲分析深度
